...
class ListBoxItem{
    ...
    public:
    virtual void copyItemTexture()=0;
};
class ListBox{
    int width;
    int height;
    SDL_Texture *texture_dialog;
    vector<ListBoxItem> dialogItems;
    public:
    ListBox();
    void draw();
    void update();
    void assignRenderer(SDL_Renderer* gRender);
    void addListItem(ListBoxItem &listItem);
};
class ResourceMeter: public ListBoxItem
{
    string imagepath;
